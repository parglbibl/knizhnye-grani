...
<section class="section">
    <div class="container" style="text-align: center;">
        ...
        <div style="display: flex; flex-direction: column; align-items: center; margin-top: 2.5rem;">
            <div style="display: flex; flex-direction: column; gap: 0.6rem; padding: 1rem 1.8rem; background: rgba(255,255,255,0.65); backdrop-filter: blur(2px); border-radius: 20px; box-shadow: 0 2px 12px rgba(0,0,0,0.03); width: 100%; max-width: 260px;">
                
                <div style="display: flex; align-items: center; gap: 1rem;">
                    <img src="/images/cube_textures/red.jpg" alt="Любовь" style="width: 28px; height: 28px; border-radius: 6px; object-fit: cover;">
                    <span style="font-weight: 500; font-size: 0.85rem; color: var(--text-secondary);">Любовь</span>
                </div>
                <div style="display: flex; align-items: center; gap: 1rem;">
                    <img src="/images/cube_textures/blue.jpg" alt="Надежда" style="width: 28px; height: 28px; border-radius: 6px; object-fit: cover;">
                    <span style="font-weight: 500; font-size: 0.85rem; color: var(--text-secondary);">Надежда</span>
                </div>
                <div style="display: flex; align-items: center; gap: 1rem;">
                    <img src="/images/cube_textures/yellow.jpg" alt="Совесть" style="width: 28px; height: 28px; border-radius: 6px; object-fit: cover;">
                    <span style="font-weight: 500; font-size: 0.85rem; color: var(--text-secondary);">Совесть</span>
                </div>
                <div style="display: flex; align-items: center; gap: 1rem;">
                    <img src="/images/cube_textures/green.jpg" alt="Добро" style="width: 28px; height: 28px; border-radius: 6px; object-fit: cover;">
                    <span style="font-weight: 500; font-size: 0.85rem; color: var(--text-secondary);">Добро</span>
                </div>
                <div style="display: flex; align-items: center; gap: 1rem;">
                    <img src="/images/cube_textures/white.jpg" alt="Память" style="width: 28px; height: 28px; border-radius: 6px; object-fit: cover;">
                    <span style="font-weight: 500; font-size: 0.85rem; color: var(--text-secondary);">Память</span>
                </div>
                <div style="display: flex; align-items: center; gap: 1rem;">
                    <img src="/images/cube_textures/orange.jpg" alt="Семья" style="width: 28px; height: 28px; border-radius: 6px; object-fit: cover;">
                    <span style="font-weight: 500; font-size: 0.85rem; color: var(--text-secondary);">Семья</span>
                </div>
            </div>
        </div>
    </div>
...
<script>
      ...
      const popupHTML = `
          <div id="bookGranPopup" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); backdrop-filter:blur(6px); z-index:9999; align-items:center; justify-content:center;">
              <div style="background:#fff; max-width:600px; width:90%; padding:2rem; border-radius:28px; position:relative; box-shadow:0 24px 70px rgba(0,0,0,0.25); max-height:90vh; overflow-y:auto;">
                  ...
                  <textarea id="popupInput" placeholder="Напиши здесь название книги и свои чувства..." style="width:100%; padding:1rem 1.2rem; border:2px solid #eee; border-radius:18px; font-family:inherit; font-size:0.95rem; min-height:80px; resize:vertical; background:#fafbfc; outline:none;"></textarea>
                  
                  <button id="popupSubmit" style="width:100%; margin-top:1rem; padding:0.9rem; background:#ff2e5a; color:#fff; border:none; border-radius:18px; font-weight:600; font-size:1rem; cursor:pointer; transition: transform 0.15s, box-shadow 0.15s; box-shadow:0 4px 16px rgba(255,46,90,0.25);">Ответить и поделиться</button>
                  
                  <p style="font-size:0.7rem; color:#999; margin-top:1rem; text-align:center; line-height:1.4;">Твой ответ увидят все, кто откроет этот квадратик</p>
              </div>
          </div>
      `;
  ...
  window.openBookGran = function(color, x, y) {
      // Без шаблонных строк — только склейка
  ...
  function loadAnswers(elementId) {
      ...
      answersRef.on('value', (snapshot) => {
          ...
          answersArray.forEach((item) => {
              ...
              div.style.cssText = 'background:#f5f0e8; border-radius:14px; padding:0.8rem 1.2rem; margin-bottom:0.8rem; box-shadow:0 2px 6px rgba(0,0,0,0.02);';
              div.innerHTML = `
                  <div style="font-size:0.9rem; color:#333; line-height:1.5;">${item.text}</div>
                  <div style="font-size:0.65rem; color:#888; margin-top:0.3rem;">${item.user || 'Аноним'} · ${item.timestamp || ''}</div>
              `;
