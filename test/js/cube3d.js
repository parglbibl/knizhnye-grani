...
if (!container) {
    ...
    function initCube(size) {
        ...
        // ============================
        // 3. Создание кубиков (чуть компактнее)
        // ============================
        const offset = 0.95; // Чуть меньше расстояние между кубиками
        const sizeCubie = 0.85; // Чуть меньше сами кубики
        const radius = 0.08;
        const segments = 4;
        ...
        // ============================
        // 7. Подсветка (Glow) — Временно упрощённая
        // ============================
        let activeGlowIds = [];
        ...
        function applyGlow() {
            ...
            // Упрощённая логика подсветки для теста
            activeGlowIds.forEach(id => {
                // Ищем кубик по ID
                allCubies.forEach(cubie => {
                    if (cubie.userData.id === id) {
                        // Подсвечиваем все грани этого кубика (для теста)
                        const faces = cubie.userData.faces;
                        const mats = cubie.material;
                        for (let i = 0; i < 6; i++) {
        ...
        // ============================
        // 10. Вращение кубика МЫШКОЙ (НОВОЕ)
        // ============================
        ...
        function onPointerMove(e) {
            ...
            if (Math.abs(deltaX) > 3 || Math.abs(deltaY) > 3) {
                isClick = false; // Если двигаем, это не клик
            }
        ...
        function onPointerUp(e) {
            ...
            if (isClick) {
                // Если это был клик без движения — открываем попап
                const coords = getXY(e);
                ...
                if (intersects.length > 0) {
                    ...
                    // Формируем ID квадратика для теста
                    const id = clickedCubie.userData.id + '_face_' + materialIndex;
                    
                    // Получаем вопрос из базы (пока заглушка)
                    const question = questionsDB[colorName + '_0_0_1'] || 'Какой вопрос ты хочешь задать этой грани?';
        ...
        // ============================
        // 11. Управление камерой и рендер
        // ============================
        let currentZoom = 4.5;
